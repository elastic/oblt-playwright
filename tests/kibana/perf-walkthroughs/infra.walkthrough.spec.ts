...
test.beforeAll('Check pod data', async ({ browser, request, log }) => {
  log.info('Checking if pod data is available in the last 24 hours');
...
test.skip('Infrastructure - Cluster Overview dashboard', async ({ dashboardPage, datePicker, headerBar, sideNav, notifications, page, log }, testInfo) => {
  ...
  await testStep('step01', stepData, page, async () => {
    ...
    log.info('Searching for the "Cluster Overview" dashboard');
...
test('K8S Aggregations dashboard', async ({ page, dashboardPage, datePicker, headerBar, log }, testInfo) => {
  let stepData: object[] = [];
  (testInfo as any).stepData = stepData;
...
test('Infrastructure - Inventory', async ({ datePicker, inventoryPage, page, log }, testInfo) => {
  ...
  await testStep('step01', stepData, page, async () => {
    ...
    log.info('Sorting by metric value and clicking on a host');
  ...
  await testStep('step02', stepData, page, async () => {
    ...
    log.info('Asserting visibility of the "Host CPU Usage" and "Host Memory Usage" visualizations');
    await Promise.race([
      Promise.all([
